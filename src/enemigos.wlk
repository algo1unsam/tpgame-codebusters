...
class Enemigo {
	...
	method cambiarImagenAtaque() {
		self.image(self.name() + direccion.nombreDireccion() + "attack.png")
	}

	method cambiarImagenCaminando(direccionxd) {
		self.image(self.name() + direccionxd.nombreDireccion() + ".png")
	}

	method atacar(objetoAATacar) {
		self.cambiarImagenAtaque()
		game.sound('meleeEnemigo.mp3').play()
...
class EnemigoRango inherits Enemigo {
	...
	override method name()="enemyrango"
	...
	method disparar() {
		if (self.puedeSeguirDisparando()) {
			game.sound('disparoEnemigo.mp3').play()
			const proyectil = new ProyectilEnemigo(parentId=self.id(),danioProyectil=danioProyectil,direccion = direccion, position = direccion.posicionSiSeMueveEnEstaDireccion(self.position()), duenio = self)
			game.addVisual(proyectil)
			game.onTick(200, "disparoEnemigo"+self.id(), { proyectil.moverse()})
...
class ProyectilEnemigo inherits Proyectil {
	...
	override method atacar() {
		if (self.estaEntablero()) {
			if (self.colicionPlayer()) {
				game.sound('colicionDisparo.mp3').play()
	...
	override method terminar() {
		game.removeTickEvent("disparoEnemigo"+parentId)
